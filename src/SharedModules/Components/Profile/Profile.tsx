  ...
  return (
    ...
    <div className="flex justify-center items-center h-[91vh] bg-sky-100">
      ...
      <div className="px-6 py-4">
        <div className="font-bold text-xl mb-2">{t("UserName")}: {`${data?.firstName.toUpperCase()} ${data?.lastName.toUpperCase()}`}</div>
        <p className="text-gray-700 text-base">{t("Role")}: {data?.role}</p>
        <p className="text-gray-700 text-base">{t("Mail")}: {data?.email}</p>
      </div>
      <div className="px-6 py-4">
